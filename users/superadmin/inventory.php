        ...
        <div class="row">
                    ...
                    <div class="card-body">
                        <form id="searchForm">
                            <div class="d-flex flex-wrap align-items-center">
                                ...
                                <button type="button" class="btn btn-outline-secondary mb-2" style="width: auto; font-size: 13px;" id="searchButton" disabled>
                            ...
                            <div class="form-row mt-3">
                                ...
                                <div class="form-group d-flex align-items-center mt-3">
                                    ...
                                    <select class="form-select" id="purposeDropdown" style="width: 300px;" name="purpose" disabled>
                            ...
                            <div class="form-row mt-3">
                                ...
                                <div class="form-group col-md-12">
                                    ...
                                    <input type="date" class="form-control" id="fromDate" name="fromDate" disabled>
                                </div>
                        ...
                        <div class="table-responsive mt-4" style="height: calc(80vh - 300px); overflow-y: auto;">
							<table class="table table-bordered table-hover" id="table-filter">
            ...
            <div class="col-md-8">
                <div class="card">
                    <div class="card-body">
                        <div style="overflow-x: auto; white-space: nowrap;">
                            ...
                            <div class="table-responsive mt-2" style="height: calc(70vh - 300px); overflow-y: auto;">
                                <table class="table table-borderless" style="margin-top: 10px;" id="table-bold">
                                    <tbody>
                                        <tr>
                                            <th class="fw-bold fs-6 fst-italic">Quantity</th>
                                            <td id="quantityValue"><!-- Value for Quantity --></td>
                                        </tr>
                                        <tr>
                                            <th class="fw-bold fs-6 fst-italic">Cost Price</th>
                                            <td id="costPriceValue"><!-- Value for Cost Price --></td>
                                        </tr>
                                        <tr>
                                            <th class="fw-bold fs-6 fst-italic">Selling Price</th>
                                            <td id="sellingPriceValue"><!-- Value for Selling Price --></td>
                                        </tr>
                                        <tr>
                                            <th class="fw-bold fs-6 fst-italic">Total Cost Price</th>
                                            <td id="totalCostPriceValue"><!-- Value for Total Cost Price --></td>
                                        </tr>
                                        <tr>
                                            <th class="fw-bold fs-6 fst-italic">Total Selling Price</th>
                                            <td id="totalSellingPriceValue"><!-- Value for Total Selling Price --></td>
                                        </tr>
                                    </tbody>
                                </table>
                            </div>
                ...
                <div class="d-flex justify-content-end">
                    <button type="button" class="btn btn-light" style="margin-left: 88%; font-size: 13px;">
                        <i class="fas fa-print"></i> Print
                    </button>
                </div>
                <div class="card">
					<div class="card-body">
						<div class="table-responsive mt-2" style="height: calc(132.5vh - 300px); overflow-y: auto;">
							<h5>List</h5>
							<table class="table table-bordered" style="margin-top: 10px;" id="table-list">
								<thead class="fw-bold fs-6 fst-italic card-header" style="background-color: #cbd1d3; color: black; position: sticky; top: 0; z-index: 1;">
									<tr>
										<th>Barcode</th>
										<th>Product Name</th>
...
<script>
    ...
    document.getElementById('searchButton').addEventListener('click', function () {
        ...
        console.log('Search button clicked, type:', type); // Debug log
        if (type === 'currentInv') {
            fetchInventorySummary();
            fetchInventoryList();
        } else {
            console.log('No action for type:', type); // Debug log
        }
    });

    function fetchInventorySummary() {
        const formData = new FormData(document.getElementById('searchForm'));
        console.log('Fetching inventory summary with formData:', Object.fromEntries(formData)); // Debug log
        fetch('fetch_inventory_summary.php', {
            method: 'POST',
            body: formData
        })
        .then(response => {
            console.log('Response status (summary):', response.status); // Debug log
            return response.json();
        })
        .then(data => {
            console.log('Received summary data:', data); // Debug log
            if (data.success) {
                document.getElementById('quantityValue').textContent = data.quantity || '0';
                document.getElementById('costPriceValue').textContent = data.costPrice || '0';
                document.getElementById('sellingPriceValue').textContent = data.sellingPrice || '0';
                document.getElementById('totalCostPriceValue').textContent = data.totalCostPrice || '0';
                document.getElementById('totalSellingPriceValue').textContent = data.totalSellingPrice || '0';
            } else {
                alert('Error fetching summary data: ' + (data.message || 'Unknown error'));
            }
        })
        .catch(error => {
            console.error('Fetch error (summary):', error); // Debug log
    ...
    function fetchInventoryList() {
        const formData = new FormData(document.getElementById('searchForm'));
        console.log('Fetching inventory list with formData:', Object.fromEntries(formData)); // Debug log
        fetch('fetch_inventory_list.php', {
            method: 'POST',
            body: formData
        })
        .then(response => {
            console.log('Response status (list):', response.status); // Debug log
            return response.json();
        })
        .then(data => {
            console.log('Received list data:', data); // Debug log
            // Target the "List" table's tbody using the unique ID
            const table = document.querySelector('#table-list');
            if (!table) {
                console.error('List table not found'); // Debug log
                return;
            }
            const tbody = table.querySelector('tbody');
            if (!tbody) {
                console.error('List table tbody not found'); // Debug log
                return;
            }
            ...
            if (data.success && data.data.length > 0) {
                ...
                tbody.innerHTML = '<tr><td class="text-center" colspan="7">No Data Available</td></tr>';
            }
        })
        .catch(error => {
            console.error('Fetch error (list):', error); // Debug log
            alert('An error occurred while fetching list data');
        });
    }
